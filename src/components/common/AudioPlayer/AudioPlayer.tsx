...
const AudioPlayer: React.FC<{ audioSrc: string; stopAudio: boolean }> = ({
  audioSrc,
  stopAudio,
}) => {
  ...
  useEffect(() => {
    if (audio) {
      audio.onloadedmetadata = () => {
        ...
        audio.onended = () => setAudioState(false);
      };
    }
  }, [audio]);
  const progress = Math.round(
    (Number(currentAudioTime) / Number(audioDuration)) * 100
  );
  ...
  return (
    <>
      <div className={styles.PlayerWrapper}>
        <div
          className={stopAudio ? styles.PlayButtonDisabled : styles.PlayButton}
          onClick={startOrStopAudio}
        >
          {!audioPlaying && (
            <svg className={styles.PlayButtonIcon} viewBox="-200 0 1200 1000">
              <path
                fill={stopAudio ? "#808080" : "#00bc8c"}
          ...
          {audioPlaying && (
            <svg className={styles.PauseButtonIcon} viewBox="0 0 47.607 47.607">
              <path
                fill="#00bc8c"
        ...
        <span className={styles.CurrentAudioTime}>
          {convertToMMSS(currentAudioTime)}
        </span>
