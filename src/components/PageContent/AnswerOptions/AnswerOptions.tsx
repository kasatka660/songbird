...
const AnswerOptions: React.FC<{
  ...
  updateScore: (attemptCount: number) => void;
  ...
  const checkAnswer = (id: number) => {
    if (
      answeredOptions.includes(id) ||
      answeredOptions.includes(currentQuestion.id)
    ) {
      return;
    }
  ...
  return (
    <ul className={`text-color ${styles.AnswerOptionList}`}>
      {BirdsData[currentCategory].map((item, key) => (
        <AnswerOption
          item={item}
          key={key}
          listItemColor={getListItemColor(item.id)}
